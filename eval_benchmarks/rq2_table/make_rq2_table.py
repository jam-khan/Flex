...
SUITE_MACROS_AGG = {
    ...
    "liquid-fixpoint": r"\liquidfixpoint",
}

# Suites merged into a single "RQ1 case studies" row in the aggregate table.
CASE_STUDY_SUITES = {"hashtable", "sorting"}
CASE_STUDY_LABEL = "RQ1 case studies"
...
def write_latex_aggregate(rows: list[dict], out_path: Path) -> None:
    ...
    L += [
        ...
        r"\bottomrule", r"\end{tabular}",
        r"\caption{RQ2 (\flux): reducing acyclic $\kappa$-variables, aggregated per suite. "
        r"For each suite we report its number of constraints (\#CHCs) and, for each of "
        r"\zap, \fusiong, and \fusiona, the number of those constraints successfully reduced "
        r"(\#reduced) together with the cost of doing so. For \zap, hb and ms are the average "
        r"heartbeats and average wall-clock time (ms) over the reduced constraints, where "
        r"heartbeats are \sclean's deterministic proxy for proof effort. For \fusiong~ and "
        r"\fusiona, \#reduced also gives coverage relative to \zap~in parentheses, and the "
        r"$\times$hb/$\times$ms columns report their average heartbeat and time cost as a "
        r"factor of \zap~over the constraints both reduce.}",
