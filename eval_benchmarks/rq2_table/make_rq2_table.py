...
SUITE_ORDER = ["flux-medium", "liquid-fixpoint", "hashtable", "sorting"]
SUITE_DISPLAY = {
...
def write_latex_aggregate(rows: list[dict], out_path: Path) -> None:
    ...
    def row(label: str, n: int, rs: list[dict]) -> str:
        ...
        gok = suite_n_ok(rs, "grind")
        aok = suite_n_ok(rs, "aesop")
        return (f"{label} & {n} & {zok}"
    ...
    L += [
        ...
        r"\bottomrule", r"\end{tabular}",
        r"\caption{RQ2: acyclic-$\kappa$ elimination — aggregate per suite. "
        r"\#ok = VCs solved; hb = geomean total proof heartbeats; "
        r"ms = geomean total proof wall time (ms). "
        r"\zap~ columns are absolute; grind/aesop columns are geomean ratio "
        r"vs.\ \zap~ over jointly-solved VCs (—\,= no jointly-solved pairs).}",
        r"\label{tab:rq2-agg}",
        r"\end{table}",
    ]
